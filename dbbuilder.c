...
char *insert_chars = "INSERT INTO chars VALUES(%d, '%q');";
...
int insert_line(sqlite3 *db, char *line, int i) {
    ...
    if(ret == SQLITE_DONE) {
        ...
        query = sqlite3_mprintf(insert_chars, i, chchar);
        if (query == NULL) {
            return 1;
        }
...
int main(int argc, char **argv) {
    ...
    sqlite3_exec(db, "CREATE INDEX i1 on chars(chchar);", NULL, NULL, NULL);
